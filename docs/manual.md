...
Возвращается 2 значения - позиция конца строки и начала следующей строки, либо конца текста s, если строк больше нет.
...
| tcp_flags_set   | установить флаги TCP. флаги представлены списком через запятую : FIN,SYN,RST,PUSH,ACK,URG,ECE,CWR                                                                                   |
...
Меняет регистр написания домена внутри заголовка `Host:`. Верхний и нижний регистр чeредуется каждый символ : `rUtRaCkEr.oRg`.
...
- arg: sni_last - добавить хост в конец списка
...
| IFACE_LAN<br>IFACE_WAN<br>IFACE_WAN6                                                                                                                                                                                                | список через пробел lan и wan интерфейсов для ipv4 и ipv6 в классическом Linux. ИНТЕРФЕЙСЫ Linux !                                            |
...
- Под стандартным режимом nfqws2 понимается то, что запускается с параметрами `NFQWS2_OPT` при включении `NFQWS2_ENABLE`. В противовес [custom скриптам](#custom-скрипты), откуда могут запускаться нестандартные, кастомные, инстансы nfqws2.
